...
class Scheduler(SchedulerTemplate):
    ...
    def refresh_schedule_display(self):
        ...
        data = anvil.server.call(
            "process_instructor_availability", selected_instructors
        )
        print("Received data from server:")
        print("Number of y_labels:", len(data["y_labels"]))
        print("Y labels received:", data["y_labels"])
        print(
            "Z values shape:",
            len(data["z_values"]),
            "x",
            len(data["z_values"][0]) if data["z_values"] else 0,
        )
    ...
    def cohort_builder_button_click(self, **event_args):
        ...
        self.cohort_schedule = anvil.server.call(
            "create_full_cohort_schedule",
        ...
        if self.cohort_schedule:
            ...
            formatted_output = (
                f"Cohort Schedule: {self.cohort_schedule['cohort_name']}\n\n"
            )
            ...
            formatted_output += f"\nSummary:\n"
            formatted_output += (
                f"Number of Students: {self.cohort_schedule['num_students']}\n"
            )
    ...
    def schedule_instructors_button_click(self, **event_args):
        instructor_allocation = anvil.server.call(
            "schedule_instructors_for_cohort",
            self.cohort,
            self.instructor1,
            self.instructor2,
        )
